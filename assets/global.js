...
if (document.getElementById('ContactForm-file')) {
  document.getElementById('ContactForm-file').addEventListener('change', function(event) {
    const url = 'https://yoker-solutions.com/wp-content/themes/yoker-child/incl/nyt-studio-file-upload.php';
    const file = event.target.files[0];
    let req = new XMLHttpRequest();
    let formData = new FormData();
  
    document.getElementById("ContactForm-file-url").required = true;
  
    formData.append("file", file);                                
    req.open("POST", url);
    req.onload = function () {
      document.getElementById("ContactForm-file-url").value = this.responseText;
    };
    req.send(formData);
